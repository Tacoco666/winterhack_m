...
# Import Parameter types for proper array handling
...
class MissionRunner(Node):
    ...
    def __init__(self) -> None:
        ...
        # Use [0.0] default to force DOUBLE_ARRAY type inference
        ...
        self.near_target = False  # Flag to track if we are close enough to search
                ...
                for i in range(0, len(raw_coords), 2):
                    if i + 1 < len(raw_coords):
                        try:
                            ...
                            self.get_logger().error(f"Invalid coordinate in block_coordinates at index {i}")
            
            if self._block_targets:
                self._target_pick_count = len(self._block_targets)
                self.get_logger().info(f"Mission 1: Initialized with {len(self._block_targets)} block targets: {self._block_targets}")
            else:
                self.get_logger().warning("Mission 1 selected but no block_coordinates provided!")
    ...
    def _on_detection(self, msg: String) -> None:
        ...
        # Mission 1 specific: Only stop for detection if we are near the target
        ...
        self.get_logger().info(
            f"Qualifying detection: {self.detected_colour}; stopping patrol/nav."
        )
    ...
    def _tick_patrol(self) -> None:
        if self.mission_id == 1:
            # --- Mission 1 Logic: Navigate to specific block coordinate ---
            if self.picked_total >= len(self._block_targets):
                self.get_logger().info("Mission 1: All targets processed.")
                self._complete_mission()
                return

            target_xy = self._block_targets[self.picked_total]
            
            # Use NavigateToPose logic
            self._update_nav_goal()
            if self._nav_goal_handle is None and self._nav_goal_future is None:
                self.get_logger().info(f"Mission 1: Navigating to block {self.picked_total + 1} at {target_xy}")
                self.near_target = False  # Reset proximity flag
            ...
            if self._nav_result_future is not None and self._nav_result_future.done():
                ...
                if success:
                    self.get_logger().info("Arrived at block location. Starting LOCATE.")
                    self._set_state(MissionPhase.LOCATE)
                else:
                    self.get_logger().warning("Failed to reach block location. Retrying...")
        else:
            # --- Default Logic: Patrol Waypoints ---
    ...
    def _tick_stop_search(self) -> None:
        if self.mission_id == 1:
            # Cancel NavigateToPose for Mission 1
            self._update_nav_goal()
            if self._nav_goal_handle is None and self._nav_goal_future is None:
                self._set_state(MissionPhase.LOCATE)
                return
            ...
            # Cancel logic using _follow_cancel_future as a generic cancel tracker
            if self._nav_goal_handle is not None and self._follow_cancel_future is None:
                self.get_logger().info("Canceling navigation goal to start localizing.")
            ...
            # Cancel FollowWaypoints for Mission 0
            self._update_follow_goal()
            if self._follow_goal_handle is None and self._follow_goal_future is None:
                self._set_state(MissionPhase.LOCATE)
                return
    ...
    def _tick_drop(self) -> None:
        ...
        if self._drop_result_future is not None and self._drop_result_future.done():
            ...
            if success:
                ...
                if self.picked_total < self._target_pick_count:
                    self.get_logger().info("More targets remain. Returning to SEARCH/NAVIGATE.")
                    self._set_state(MissionPhase.PATROL_SEARCH)
                else:
                    self._complete_mission()
            else:
    ...
    def _send_nav_goal_xy(self, x: float, y: float, feedback_callback=None) -> None:
        """Helper to send a NavigateToPose goal to (x, y) with optional feedback."""
        if not self._nav_client.wait_for_server(timeout_sec=0.0):
            return
        stamp = self.get_clock().now().to_msg()
        goal = NavigateToPose.Goal()
        goal.pose = make_pose_stamped(
            x, y, 0.0, self._map_frame, stamp
        )
    ...
    def _on_nav_feedback(self, feedback_msg) -> None:
        """Monitor distance to target and enable detection when close."""
        feedback = feedback_msg.feedback
        # Check if distance remaining is less than 0.25m
        if feedback.distance_remaining < 0.25:
            if not self.near_target:
                 self.get_logger().info("Approaching target (within 25cm). Enabling detection.")
