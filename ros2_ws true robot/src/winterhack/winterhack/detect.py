...
class ColorDetector(Node):
    def __init__(self):
        ...
        default_camera_info_topic = (
            depth_topic.replace("image_raw", "camera_info")
            if "image_raw" in depth_topic
            else "/ascamera/camera_publisher/depth0/camera_info"
        )
        ...
        self.camera_info = {
            ...
            'width': 640,
            'height': 400,
            'scale_factor': self.depth_scale  # Depth scale factor (mm to meters if needed)
        }
        self._camera_info_ready = False
        self.depth_encoding = None
        self.declare_parameter("camera_frame_id", "depth_camera_link")
    ...
    def camera_info_callback(self, msg: CameraInfo) -> None:
        self.camera_info['fx'] = float(msg.k[0])
        self.camera_info['fy'] = float(msg.k[4])
        self.camera_info['cx'] = float(msg.k[2])
        self.camera_info['cy'] = float(msg.k[5])
        self.camera_info['width'] = int(msg.width)
        self.camera_info['height'] = int(msg.height)
        if not self._camera_info_ready:
            self._camera_info_ready = True
            self.get_logger().info(
                f"CameraInfo updated: fx={self.camera_info['fx']:.3f}, "
                f"fy={self.camera_info['fy']:.3f}, "
                f"cx={self.camera_info['cx']:.3f}, "
                f"cy={self.camera_info['cy']:.3f}, "
