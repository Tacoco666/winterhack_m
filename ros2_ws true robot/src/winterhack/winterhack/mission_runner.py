...
class MissionRunner(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("state_transition_delay_s", 0.0)
        ...
        self._pending_state: Optional[MissionPhase] = None
        self._pending_state_until: Optional[float] = None
    ...
    def _apply_state_change(self, new_state: MissionPhase) -> None:
        ...
        self.get_logger().info(f"状态切换：{current_label} -> {next_label}")
    ...
    def _set_state(self, new_state: MissionPhase) -> None:
        ...
        if new_state == MissionPhase.IDLE:
            ...
            self._pending_state_until = None
        ...
        self._pending_state_until = time.monotonic() + delay_s

    def _maybe_apply_pending_state(self) -> bool:
        if self._pending_state is None or self._pending_state_until is None:
            return False
        if time.monotonic() < self._pending_state_until:
            return True
        pending = self._pending_state
        self._pending_state = None
        self._pending_state_until = None
        self._apply_state_change(pending)
        return False
    ...
    def _tick(self) -> None:
        if self._maybe_apply_pending_state():
            return
        if self._state == MissionPhase.IDLE:
            return
